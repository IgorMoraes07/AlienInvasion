# -*- coding: utf-8 -*-
"""
Created on Sat Jan  7 15:51:21 2023
...
def check_bullet_alien_collisions(ai_settings, screen, stats, sb, ship, aliens, bullets):
    ...
    # Remove qualquer projétil e alienígenas que tenham colidido
    ...
    if len(aliens) == 0:
        # Se a frota toda for destrída, inicia-se um novo nível
        bullets.empty()
...
def get_number_rows(ai_settings, ship_height, alien_height):
    """Determina o número de linhas com aienigenas que cabem na tela."""
...
def create_alien(ai_settings, screen, aliens, alien_number, row_number):
    # Cria uma alienigena e o posiciona na linha
...
def check_aliens_bottom(ai_settings, screen, stats, sb, ship, aliens, bullets):
    ...
    for alien in aliens.sprites():
        if alien.rect.bottom >= screen_rect.bottom:
            # Trata esse caso do mesmo jeito que é feito quando a espaçonave é atingida
            ship_hit(ai_settings, screen, stats, sb, ship, aliens, bullets)
            break

def update_aliens(ai_settings, screen, stats, sb, ship, aliens, bullets):
    """Verifica se a frota está em uma das bordas 
        e então atuaiza às posições de todos os alienigenas da frota."""
    check_fleet_edges(ai_settings, aliens)
    aliens.update()
    
    # Verifica se houveram colisões entre alienigenas e a espaçonave
